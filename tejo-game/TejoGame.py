...
from game.ui_imgui import UISystemImGui
...
class TejoGame(PUJ_Ogre.BaseApplicationWithVTK):
    ...
    def _loadScene(self):
        ...
        # Inicializar UI con ImGui
        self.ui_system = UISystemImGui(
            self.getRenderWindow(),
            self.m_SceneMgr,
            self.main_camera
        )
    ...
    def _create_board_visual(self):
        limites = [-BOARD_LENGTH/2, BOARD_LENGTH/2, -BOARD_WIDTH/2, BOARD_WIDTH/2]
        ...
        rotation = Ogre.Quaternion(math.cos(angle_rad/2), 0, 0, math.sin(angle_rad/2))
        self.board_node.setOrientation(rotation)
        
        disc_radius = 0.075
        disc_height = 0.002
        disc_data = self._cone(disc_radius, disc_height, 40, top_radius=disc_radius)
        self.disc_node = self._createManualObject(disc_data, 'center_disc', 'BaseWhiteNoLighting')
    ...
    def _create_mechas(self):
        ...
        for i, pos in enumerate(mecha_positions_local):
            ...
            mecha_node = self._createManualObject(mecha_data, f'mecha_{i}', 'yellow_material')
    ...
    def _crear_tejo_para_lanzar(self):
        ...
        self.ui_system.update_score_display(self.game_state.scores[0], self.game_state.scores[1])
        ...
        tejo_node = self._createManualObject(tejo_data, self.current_tejo_name, material)
    ...
    def _procesar_puntuacion(self):
        ...
        distance_to_disc = math.sqrt((pos[0] - self.disc_position[0])**2 + (pos[2] - self.disc_position[2])**2)
        ...
        for detail in details:
            print(f"  - {detail}")
        print(f"  - Distancia al bocin: {distance_to_disc:.3f}m")
        print("="*50)
        ...
        if proximity_result:
            ...
            print(f"\nMANO! Equipo {'A' if winning_team == 0 else 'B'} tiene el tejo mas cercano (+1 punto)\n")
            team_scores = [self.game_state.scores[0], self.game_state.scores[1]]
            self.ui_system.update_score_display(team_scores[0], team_scores[1])
    ...
    def frameRenderingQueued(self, evt):
        ...
        # Renderizar ImGui UI
        self.ui_system.frame_update(evt.timeSinceLastFrame)
        self.ui_system.new_frame()
